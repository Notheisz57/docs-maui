...
description: "The .NET MAUI BindableLayout is a layout that generates its content by binding to a collection of items, with the option to set the appearance of each item with a DataTemplate."
ms.date: 01/06/2022
---
...
All of these properties can be attached to the `AbsoluteLayout`, `FlexLayout`, `GridLayout`, `RelativeLayout`, and `StackLayout` classes, which all derive from the `Layout<T>` class.

The `Layout<T>` class exposes a `Children` collection, to which the child elements of a layout are added. When the `BinableLayout.ItemsSource` property is set to a collection of items and attached to a `Layout<T>`-derived class, each item in the collection is added to the `Layout<T>.Children` collection for display by the layout. The `Layout<T>`-derived class will then update its child views when the underlying collection changes. For more information about the .NET MAUI layout cycle, see [Creating a Custom Layout](~/xamarin-forms/user-interface/layouts/custom.md).
...
For more information about data templates, see [.NET MAUI Data Templates](~/xamarin-forms/app-fundamentals/templates/data-templates/introduction.md).
...
public class TechItemTemplateSelector : DataTemplateSelector
{
    ...
    {
        return (string)item == ".NET MAUI" ? XamarinFormsTemplate : DefaultTemplate;
    }
}
```
...
For more information about data template selectors, see [Creating a .NET MAUI DataTemplateSelector](~/xamarin-forms/app-fundamentals/templates/data-templates/selector.md).
...
[![Screenshot of a bindable layout string empty view.](bindable-layouts-images/emptyview-string.png "Bindable layout string empty view."](bindable-layouts-images/emptyview-string-large.png#lightbox "Bindable layout string empty view")
...
[![Screenshot of a bindable layout empty view with multiple views.](bindable-layouts-images/emptyview-views.png "Bindable layout empty view."](bindable-layouts-images/emptyview-views-large.png#lightbox "Bindable layout empty view")
...
[![Screenshot of a bindable layout empty view template.](bindable-layouts-images/emptyviewtemplate.png "Bindable layout empty view template."](bindable-layouts-images/emptyviewtemplate-large.png#lightbox "Bindable layout empty view template")
...
[![Screenshot of empty view choice at runtime.](bindable-layouts-images/emptyview-runtime.png "Bindable layout empty view runtime choice."](bindable-layouts-images/emptyview-runtime-large.png#lightbox "Bindable layout empty view runtime choice")
